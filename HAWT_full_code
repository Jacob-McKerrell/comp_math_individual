...
% F_x
f_HAWT = -A .* exp(-(X-x_0).^2/(2*sig_x^2)) .* exp(-(Z-z_0).^2/(2*sig_z^2));

% FFT along x (columns) of force
f_hat = fft(f_HAWT,[],2);   % FFT along x, size (N_z+1 x N_x)
...
for j = 1:N_x
    ...
    main_diag(:) = a(k);
    A_mat = spdiags([off_diag main_diag off_diag], -1:1, N, N);
    A_mat(N,N-1) = 2*b; % Neuman conditions
...
set(gca, 'YDir', 'normal');
xlabel('$x$ (m)','interpreter','latex'); ylabel('$z$ (m)','interpreter','latex');
title('Contour of $U_0+u(x,z)$','interpreter','latex', "FontSize",  24);
colorbar;
clim([13.5 20.1]);
ylim([0 500])
fontsize(24,"points")
...
%set(gca, 'YDir', 'normal');
xlabel('$x$ (m)','interpreter','latex'); ylabel('$z$ (m)','interpreter','latex');
title('Contour of $u(x,z)$','interpreter','latex');
hold on 
F_U = scatteredInterpolant(X(:), Z(:), U(:), 'linear', 'none');
F_W = scatteredInterpolant(X(:), Z(:), W(:), 'linear', 'none');
...
% Power Calculations
numerator_HAWT = trapz(z, trapz(x, f_HAWT .* U, 2));
HAWT_denominator = 0.5*1.20769*(top-bottom)*U0^3;

%Calculate efficiency
HAWT_efficiency = numerator_HAWT / HAWT_denominator;
