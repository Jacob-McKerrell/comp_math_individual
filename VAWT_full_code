...
N_z = 1000;     % number of z intervals
delta = 1;  %Sensitivity of tanh approximation
% Grids
...
% Modification of f_x function

bottom = 125;
top = 175;
f = -A .* exp(-(X-x_0).^2/(2*sig_x^2)) .* (tanh((Z-bottom)/delta) - tanh((Z-top)/delta));

%Calculate new A_VAWT
...
minus_df = A_VAWT*exp(-((X-x_0).^2/(2*sig_x^2))) * (1/delta).*(sech((Z-bottom)/delta).^2 - (sech((Z-top)/delta).^2));
minus_df_hat = fft(minus_df, [],2);
...
w_hat = zeros(size(minus_df_hat));  % preallocate space
...
for j = 1:N_x
    ...
    %Compute d/dz analytically
    F_k = minus_df_hat(1:N, j);
...
dx = L_x / (N_x - 1); % Define the horizontal step size

x = linspace(0, L_x, N_x);
u = U0+cumtrapz(x, -dwdz, 2); 
...
title('Contour of $w(x,z)$','interpreter','latex', "FontSize",16);
colorbar;
...
set(gca, 'YDir', 'normal');
xlabel('$x$ (m)','interpreter','latex'); ylabel('$z$ (m)','interpreter','latex');
title('Contour of $U_0+u(x,z)$','interpreter','latex', "FontSize",16);
colorbar;
clim([13.5 20.1]);
ylim([0 500])
fontsize(24,"points")
...
%set(gca, 'YDir', 'normal');
xlabel('$x$ (m)','interpreter','latex'); ylabel('$z$ (m)','interpreter','latex');
title('Contour of $u(x,z)$','interpreter','latex',"FontSize",20);
hold on 
F_U = scatteredInterpolant(X(:), Z(:), U(:), 'linear', 'none');
F_W = scatteredInterpolant(X(:), Z(:), W(:), 'linear', 'none');
...
% Power Calculations
numerator_VAWT = trapz(z, trapz(x, f_VAWT .* U, 2));
VAWT_denominator = 0.5*1.20769*(top-bottom)*U0^3;

%Calculate efficiency
VAWT_efficiency = numerator_VAWT / VAWT_denominator;
